testDataSourceQuanteda <- function() {

  init <- function() {
    source('./test/testFunctions/LogTest.R')
    DataSourceTest <<- LogTest$new()

    qt <- readtext::readtext("./test/testData/input/*.txt")
    qc <- quanteda::corpus(qt)
    quanteda::docvars(qc, "name") <- c("Blogs", "News", "Twitter")
    quanteda::docvars(qc, "source") <- c("Swiftkey")

    return(qc)
  }

  test0 <- function(qc) {
    test <- "test0: Comvert quanteda corpus"
    cat(paste0("\n",test, " Commencing\n"))
...
className <- "DataSourceQuanteda"

testDataSourceQuanteda()
