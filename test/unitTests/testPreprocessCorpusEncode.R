testPreprocessCorpusEncodeStrategy <- function() {
  ...
  init <- function() {
    ...
    PreprocessCorpusEncodeStrategyTest <<- LogTest$new()
  }
  ...
  test0 <- function() {
    ...
    # Logit
    PreprocessCorpusEncodeStrategyTest$logs(className = className, methodName = "initialize", msg = paste("Successfully initialized Corpus"))
  ...
  test1 <- function(cb) {
    ...
    lapply(seq_along(docs1), function(d) {
      ...
      #stopifnot(!identical(t1, t2)) may not be different if encoding errors were not encountered.
      print(paste0("Length of old content is ", length(t1)))
      print(paste0("Length of new content is ", length(t2)))
    })

    # Logit
    PreprocessCorpusEncodeStrategyTest$logs(className = className, methodName = "process", msg = paste("Successfully processed repair"))
    PreprocessCorpusEncodeStrategyTest$logs(className = className, methodName = "process", msg = paste("Successfully returned repaired object"))
  ...
  testn <- function(news) {
    ...
    # Logit
    PreprocessCorpusEncodeStrategyTest$logs(className = className, methodName = "content", msg = paste("Successfully instantiated document with file"))
...
testPreprocessCorpusEncodeStrategy()
