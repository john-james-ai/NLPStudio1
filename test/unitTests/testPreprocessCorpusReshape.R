testPreprocessCorpusReshape <- function() {
  ...
  init <- function() {
    ...
    PreprocessCorpusReshapeTest <<- LogTest$new()
  }

  test0 <- function() {
    test <- "test0: Corpus: Instantiation corpus to reshape"
    cat(paste0("\n",test, " Commencing\n"))
    ...
    # Logit
    PreprocessCorpusReshapeTest$logs(className = className, methodName = "initialize", msg = paste("Successfully initialized Corpus"))
  ...
  test1 <- function(cb) {
    ...
    # Logit
    PreprocessCorpusReshapeTest$logs(className = className, methodName = "process", msg = paste("Successfully processed repair"))
    PreprocessCorpusReshapeTest$logs(className = className, methodName = "process", msg = paste("Successfully returned repaired object"))
  ...
  testn <- function(news) {
    ...
    # Logit
    PreprocessCorpusReshapeTest$logs(className = className, methodName = "content", msg = paste("Successfully instantiated document with file"))
...
className <- "PreprocessCorpusReshape"

testPreprocessCorpusReshape()
